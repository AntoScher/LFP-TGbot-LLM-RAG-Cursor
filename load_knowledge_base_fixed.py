...
def load_documents():
    ...
    # Get all markdown and text files (excluding system_prompt.txt which is now in root)
...
def main():
    ...
    try:
        ...
        # Проверяем, что база создалась корректно
        collection = vectordb._collection
        if collection:
            count = collection.count()
            logger.info(f"Успешно загружено {count} фрагментов в базу данных {persist_dir}")
        else:
            logger.error("Ошибка: коллекция не создана")
