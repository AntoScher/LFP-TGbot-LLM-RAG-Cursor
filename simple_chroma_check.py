import os
import sys
import chromadb
from chromadb.config import Settings
import logging

# Configure logging to output to console with UTF-8 encoding
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger(__name__)

def check_chroma():
    """Check the status and contents of the Chroma database"""
    db_path = "./chroma_db"
    
    if not os.path.exists(db_path):
        logger.error(f"Chroma database not found at {os.path.abspath(db_path)}")
        return
    
    try:
        # Initialize Chroma client
        logger.info("Initializing Chroma client...")
        client = chromadb.PersistentClient(
            path=db_path,
            settings=Settings(anonymized_telemetry=False)
        )
        
        # List all collections
        collections = client.list_collections()
        logger.info(f"Found {len(collections)} collection(s) in the database.")
        
        if not collections:
            logger.warning("No collections found in the database.")
            return
        
        # Check each collection
        for collection in collections:
            logger.info(f"\nCollection: {collection.name}")
            
            # Get collection details
            col = client.get_collection(collection.name)
            count = col.count()
            logger.info(f"- Document count: {count}")
            
            if count == 0:
                logger.warning("  Collection is empty.")
                continue
            
            # Get a sample of documents
            try:
                logger.info("  Fetching sample documents...")
                results = col.get(limit=min(3, count))
                
                if not results or 'documents' not in results or not results['documents']:
                    logger.warning("  No documents found in the collection.")
                    continue
                
                # Display sample documents
                logger.info("  Sample documents:")
                for i, doc in enumerate(results['documents']):
                    print(f"\n  --- Document {i+1} ---")
                    try:
                        # Print document content with encoding handling
                        print("  Content (first 300 chars):")
                        print("  " + "-" * 50)
                        print(str(doc)[:300])
                        print("  " + "-" * 50)
                    except Exception as e:
                        logger.error(f"  Error displaying content: {e}")
                    
                    # Print metadata if available
                    if 'metadatas' in results and results['metadatas'] and i < len(results['metadatas']):
                        print("  Metadata:")
                        print(f"  {results['metadatas'][i]}")
                
            except Exception as e:
                logger.error(f"  Error fetching documents: {e}")
    
    except Exception as e:
        logger.error(f"Error initializing Chroma client: {e}")
        import traceback
        logger.error(traceback.format_exc())

if __name__ == "__main__":
    # Set console to use UTF-8 encoding
    import sys
    import codecs
    if sys.stdout.encoding != 'utf-8':
        sys.stdout = codecs.getwriter('utf-8')(sys.stdout.buffer, 'strict')
    if sys.stderr.encoding != 'utf-8':
        sys.stderr = codecs.getwriter('utf-8')(sys.stderr.buffer, 'strict')
    
    check_chroma()
